    ...
    async function AICHAT(client, message, guild_settings, setups) {
        try{
            ...
            if(message.channel.id == chatbot){
              ...
              try{
                fetch(`http://api.brainshop.ai/get?bid=160446&key=cWbix6pBWbEINr7d&uid=1&msg=${encodeURIComponent(message)}`)
                .then(res => res.json())
                .then(data => {
                  translate(data.cnt, { to: 'zh-TW' }).then(tlres => {
				  	message.reply({ content: tlres.text, allowedMentions: { repliedUser: true }})
				  }).catch(err => {
				    console.error(err);
				  });
                });
